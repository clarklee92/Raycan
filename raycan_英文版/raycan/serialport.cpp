...
BEGIN_MESSAGE_MAP(Cserialport, CDialogEx)
	...
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_BUTTON3, &Cserialport::OnBnClickedabort)
END_MESSAGE_MAP()
...
void Cserialport::OnClickedOpencomm()
{
	...
	{
		MessageBox(_T("No free serial port!"));
		return;
	}
	...
	{
		MessageBox(_T("Serial port open failed!"));
		return;
	}
	else{
		flag_Initialization = true;
		MessageBox(_T("Serial port open successfully!"));
...
void Cserialport::OnClickedReadboardnum()
{
	...
	{
		MessageBox(_T("Serial port is not initialized!"));
		return;
	}
	else{
		if (processcount != 0 && processcount != 288)
		{
			MessageBox(_T("Serial port is busy!"));
			return;
		}
		else{
			...
			if (Recv.GetLength() == 12 && Recv.Left(2) == "01" && Recv.Mid(2, 4) == "0000")
			{
				...
				if (Recv_ZL_CZ == "010000" && Recv_ZL_JY == Recv.Mid(10, 2))
				{
					...
					UpdateData(FALSE);
					m_numnote.SetWindowText(_T("Successfully read"));
				}
				else
				{
					m_boardnum = "null";
					UpdateData(FALSE);
					MessageBox(_T("unexpected command received，please check!"));
					return;
				}
			}
			else
...
void Cserialport::OnBnClickedWriteboardnum()
{
	...
	if (hCom == INVALID_HANDLE_VALUE || m_commport.GetCount() == 0 || flag_Initialization == false)
	{
		...
		if (processcount != 0 && processcount != 288)
		{
			...
			{
				MessageBox(_T("Please input the NUM!"));
				return;
			}
			else{
				int flag = 0;
				for (int i = 0; i < m_boardnum.GetLength(); i++)
				{
					if (m_boardnum.GetAt(i) > ('0' - 1) && m_boardnum.GetAt(i) < ('9' + 1))
					{
						...
						MessageBox(_T("Please input the correct NUM!"));
						return;
					}
				}
				if (flag == m_boardnum.GetLength())
				{
					...
					if (boardrange >= 0 && boardrange < 10000)
					{
						...
						if (Recv == Recv_ZL)
						{
							//m_boardnum = _T("已写入");
							//UpdateData(FALSE);
							m_numnote.SetWindowText(_T("Successfully write"));
						}
						else{
							m_numnote.SetWindowText(_T("Failed,please retry!"));
							return;
						}
					}
					else{
		...
		if (hCom == INVALID_HANDLE_VALUE || m_commport.GetCount() == 0 || flag_Initialization == false)
		{
			...
			if (processcount != 0 && processcount != 288)
			{
				...
				if (m_ipaddress.IsBlank())
				{
					...
					{
						m_ipnote.SetWindowText(_T("Successfully write"));
						//m_ipaddress.SetAddress(0, 0, 0, 0);
					}
					else{
						m_ipnote.SetWindowText(_T("Failed,please retry!"));
						return;
					}
				}
			}
									...
									if (Recv_ZL_CZ == "010000" && Recv_ZL_JY == Recv3.Mid(10, 2))
									{
										...
										if (Recv4.GetLength() == 12 && Recv4.Left(2) == "01" && Recv4.Mid(2, 4) == "0000")
										{
											...
											if (Recv_ZL_CZ == "010000" && Recv_ZL_JY == Recv4.Mid(10, 2))
											{
												...
												m_ipnote.SetWindowText(_T("Successfully read"));
											}
											else
											{
												MessageBox(_T("unexpected command received，please check!"));
	...
	void Cserialport::OnClickedClosecomm()
	{
		...
		if (processcount != 0 && processcount != 288)
		{
			MessageBox(_T("Serial port is busy,Please wait!"));
			return;
		}
		else{
			CloseHandle(hCom);
	...
	void Cserialport::OnClickedWrite()//单一阈值写入
	{
		...
		if (hCom == INVALID_HANDLE_VALUE || m_commport.GetCount() == 0 || flag_Initialization == false)
		{
			...
			if (processcount!=0&&processcount!=288)
			{
				...
				if (Recv == Recv_ZL)
				{
					MessageBox(_T("Write threshold successfully!"));
				}
				else
				{
					MessageBox(_T("unexpected command received，please check!"));
	...
	void Cserialport::OnClickedSenddata()
	{
		...
		if (hCom == INVALID_HANDLE_VALUE || m_commport.GetCount() == 0 || flag_Initialization == false)
		{
			...
			CButton *Stopsenddata = (CButton *)GetDlgItem(IDC_ABORT);
	...
	void Cserialport::OnBnClickedabort()
	{
		// TODO:  在此添加控件通知处理程序代码
		sendflag = true;
		CButton *Stopsenddata = (CButton *)GetDlgItem(IDC_ABORT);
