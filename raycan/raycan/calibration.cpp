...
BOOL Ccalibration::InitSocket()
{
	...
	if (ret != 0)
	{
		MessageBox(_T("Failed to initialize the network protocol!"));
		return FALSE;
	}
	...
	if (SOCKET_ERROR == retval)
	{
		...
		MessageBox(_T("Network port binding failed!"));
		return FALSE;
	}
	return TRUE;
}
...
bool Ccalibration::OnSaveas()
{
	...
	if (FileDlg.DoModal() == IDOK)
	{
		...
		if (!m_FileFind.FindFile(g_strProjectPath)) //路径不存在则创建该路径
		{
			...
			{
				AfxMessageBox(_T("Project path creation failed, please try again！"));
				return false;
			}
		}
		return true;
	...
	else
	{
	    ...
	    while (TRUE)
		{
			...
			if (!select(sock + 1, &readfds, NULL, NULL, &tv))
			{
				...
				MessageBox(_T("Please check your network connection and IP settings！"));
				CButton *Begaintime = (CButton *)GetDlgItem(IDC_BEGAINTIME);
				Begaintime->EnableWindow(TRUE);
				CButton *stopbysize = (CButton *)GetDlgItem(IDC_STOPBYSIZE);
				stopbysize->EnableWindow(TRUE);
...
void Ccalibration::OnClickedStopbysize()
{
	...
	{
		MessageBox(_T("Please input the amount！"));
		return;
	}
	Stopbysizeflag = true;
	OnClickedBegaintime();	
...
void Ccalibration::OnClickedStopbytime()
{
	...
	{
		MessageBox(_T("Please input the time！"));
		return;
	}
	Stopbytimeflag = true;
	OnClickedBegaintime();
